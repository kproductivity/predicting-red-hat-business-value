...
fit.rf1 <- h2o.randomForest(y = y, x = x1,
...
fit.rf2 <- h2o.randomForest(y = y, x = x2,
...
predict1 <- h2o.predict(fit.rf1, test1)[,1]
predict1 <- cbind(as.data.frame(test1$activity_id), as.data.frame(predict1))
predict2 <- h2o.predict(fit.rf2, test2)[,1]
...
write.csv(prediction, "submission.rf1.csv", row.names = FALSE, quote = FALSE)
