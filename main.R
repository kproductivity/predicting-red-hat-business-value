...
train$timelag <- as.Date(train$date) - as.Date(train$pdate) #days activity takes place after user starts
test$timelag <- as.Date(test$date) - as.Date(test$pdate)    #days activity takes place after user starts
...
write.csv(prediction, "submission.gbm2.csv", row.names = FALSE, quote = FALSE)
